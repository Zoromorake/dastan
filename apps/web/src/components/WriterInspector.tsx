...
export function WriterInspector({
	...
	const collapsedAsideClass = isDark
		? 'flex min-h-0 w-12 shrink-0 flex-col overflow-hidden border-l border-slate-700 bg-slate-800 transition-all duration-200'
		: 'flex min-h-0 w-12 shrink-0 flex-col overflow-hidden border-l border-stone-300 bg-[#f6f2ea] transition-all duration-200';
	const expandedAsideClass = isDark
		? 'flex min-h-0 w-[22rem] shrink-0 flex-col overflow-hidden border-l border-slate-700 bg-slate-800 transition-all duration-200 max-md:fixed max-md:inset-y-0 max-md:right-0 max-md:z-40 max-md:w-[min(22rem,100vw)] max-md:shadow-xl'
		: 'flex min-h-0 w-[22rem] shrink-0 flex-col overflow-hidden border-l border-stone-300 bg-[#f6f2ea] transition-all duration-200 max-md:fixed max-md:inset-y-0 max-md:right-0 max-md:z-40 max-md:w-[min(22rem,100vw)] max-md:shadow-xl';
	...
	return (
		<>
			<button
				...
				onClick={onToggleCollapsed}
			/>
			<aside className={expandedAsideClass}>
					...
					<>
						...
						<CollapsibleSection isDark={isDark} title="Mores and Continueds">
							<p className={`mb-3 text-[11px] ${isDark ? 'text-slate-400' : 'text-stone-600'}`}>
								(MORE) and (CONT&apos;D) settings apply in PDF/print export. Live page preview does not insert them yet — see future-tasks.md.
							</p>
						...
						<CollapsibleSection isDark={isDark} defaultOpen={false} title="Document">
							...
							<InspectorRow isDark={isDark} label="Lock pages through">
								<input
									...
									min={0}
									type="number"
									value={documentLayout.lockedPageCount}
									onChange={(event) =>
										onLayoutChange({
											lockedPageCount: Math.max(0, Number.parseInt(event.target.value, 10) || 0),
										})
									}
								/>
							</InspectorRow>
