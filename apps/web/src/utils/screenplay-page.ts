...
import type { ScreenplayZoomLevel } from '../types';
import { DEFAULT_PAGE_MARGINS, SCRIPT_PAGE_HEIGHT_PX, SCRIPT_PAGE_WIDTH_PX } from './screenplay-page-constants';

export const ZOOM_MULTIPLIERS: Record<ScreenplayZoomLevel, number> = {
	90: 0.9,
	100: 1,
	110: 1.1,
	125: 1.25,
...
export function computePageFitScale(availableWidth: number, pageWidth = SCRIPT_PAGE_WIDTH_PX): number {
	if (availableWidth <= 0 || availableWidth >= pageWidth) {
		return 1;
	}

	return Math.max(0.45, availableWidth / pageWidth);
}
