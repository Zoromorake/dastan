...
const aiProviders: AiProvider[] = ['openai', 'anthropic', 'google', 'openrouter'];
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!apiKey) {
                      setAiTestStatus(`Add an API key for ${AI_PROVIDER_LABELS[provider]} first.`);
                      return;
                    }
          ...
          {activeTab === 'notifications' ? (
            <div className="mx-auto grid max-w-2xl gap-3">
              ...
              <ToggleRow
                ...
                checked={settings.collaboratorMentions}
                theme={ui}
                onChange={(checked) => setSettings((current) => ({ ...current, collaboratorMentions: checked }))}
              />
              <ToggleRow
                label="Product updates newsletter"
                checked={settings.newsletter}
                theme={ui}
                onChange={(checked) => setSettings((current) => ({ ...current, newsletter: checked }))}
              />
            </div>
          ) : null}
