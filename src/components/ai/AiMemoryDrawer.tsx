...
export function AiMemoryDrawer({ open, isDark, documentId, memories, onClose, onMemoriesChange }: AiMemoryDrawerProps) {
	const [draft, setDraft] = useState('');
	const [scope, setScope] = useState<'global' | 'document'>('document');
			...
			<div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
				<div className="space-y-2">
					...
					<div className="grid grid-cols-2 gap-2">
						{(['document', 'global'] as const).map((option) => (
							<button
								key={option}
