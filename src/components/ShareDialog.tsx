...
export function ShareDialog({ open, title, documentId, onClose, onOpenAddressBook }: ShareDialogProps) {
  ...
  const shareLink = documentId ? `${window.location.origin}/script/${documentId}` : '';
            ...
            {shareLink ? (
              <Card className="mt-4">
                <CardHeader>
                  ...
                  <CardDescription>Copy a direct link to this script on this device.</CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3">
        ...
        <DialogFooter className="px-6" showCloseButton={false}>
          ...
          <Button
            ...
            onClick={() => {
              ...
              const inviteNote = note.trim().length > 0 ? `\n\nNote: ${note.trim()}` : '';
              const payload = `Invite prepared for ${names} with ${permission} permission.${inviteNote}\n\nCloud delivery will activate once sync is enabled.`;
