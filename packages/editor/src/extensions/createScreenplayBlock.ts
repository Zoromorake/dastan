    ...
    addAttributes() {
      ...
      return {
        dualDialogueSide: {
          default: null,
          parseHTML: (element) => {
            const side = element.getAttribute('data-dual-dialogue-side');
            if (side === 'left' || side === 'right') {
              return side;
            }

            return element.getAttribute('data-dual-dialogue') === 'true' ? 'left' : null;
          },
          renderHTML: (attributes) => {
            if (attributes.dualDialogueSide !== 'left' && attributes.dualDialogueSide !== 'right') {
              return {};
            }

            return { 'data-dual-dialogue-side': attributes.dualDialogueSide };
          },
        },
      };
    },
